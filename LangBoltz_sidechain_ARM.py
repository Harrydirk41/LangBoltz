...
class TransformerBlock(nn.Module):
    ...
    def forward(self,x_seq,x_struc,mask,index = 0):
        if index == 0:
            r1_seq,r1_struc = self.attn(x_seq, x_struc,mask,index = index)
...
class Transformerstack(nn.Module):
    ...
    def forward(self,x_seq,x_struc,mask,index = 0):
        for block in self.blocks:
            x_seq,x_struc = block(x_seq,x_struc,mask,index = index)
...
class ESMDynamics(nn.Module):
    def __init__(self,d_model,n_heads,bias = False,qk_layernorm=True,n_layers=24):
        ...
        self.torsion_head = RegressionHead(d_model, self.n_bins + 5)
        self.seq_embed = nn.Embedding(64, d_model)
        self.struc_embed = nn.Embedding(4096 + 5, d_model)
        self.torsion_embed = nn.Embedding(self.n_bins + 5, d_model)
        self.torsion_pos_embed = nn.Embedding(5, d_model)

    def forward(self,seq_tokens,struc_tokens,torsion_tokens,mask,index=0):
        N_seq = seq_tokens.shape[1]
        x_seq = self.seq_embed(seq_tokens)
        x_struc = self.struc_embed(struc_tokens)
        x_torsion = self.torsion_embed(torsion_tokens)
        x_input_seq = x_seq + x_struc
        torsion_pos = torch.arange(5).unsqueeze(0).repeat(seq_tokens.shape[0],seq_tokens.shape[1]).to(x_seq.device)
        torsion_pos_embed = self.torsion_pos_embed(torsion_pos)
        x_input_torsion = x_torsion + torsion_pos_embed[:,index:index+1]
        x_torsion_out,x_torsion_out_embed = self.transformer(x_input_seq,x_input_torsion,mask,index=index)
...
class MultiHeadAttention(nn.Module):
    ...
    def _apply_rotary(self, q: torch.Tensor, k: torch.Tensor,use_struc = True,N_seq = None,N_seg = 5, index = 0):
        q = q.unflatten(-1, (self.n_heads, self.d_head))
        k = k.unflatten(-1, (self.n_heads, self.d_head))
        if use_struc:
            position_res = torch.arange(N_seq).to(q.device)
            position_res = position_res.repeat_interleave(N_seg)
            q, k = self.rotary_time(q, k,positions=position_res[index:index + 1])
        else:
    ...
    def forward(self, x_seq, x_struc,mask,index = 0):
        ...
        query_BLD_struc, key_BLD_struc = self._apply_rotary(query_BLD_struc, key_BLD_struc,use_struc = True,N_seq = N_seq,index=index)
...
class ESM3LightningModule(pl.LightningModule):
    def __init__(self, model):
        ...
        self.model = model
    def forward(self, seq_tokens, struc_tokens,torsion_tokens,mask,index=1):
        torsion_logits = self.model(seq_tokens=seq_tokens, struc_tokens=struc_tokens,torsion_tokens=torsion_tokens,mask = mask,index = index)
        return torsion_logits
        
    def distance_attention_mask(self,coords, min_dist=1.0, max_dist=12.0):
        """
        Create an attention mask based on pairwise distances,
        including self-attention (diagonal entries).
    
        Args:
            coords: (N_batch, N_res, 3) tensor of positions
            min_dist, max_dist: distance range for valid pairs (Å)
    
        Returns:
            mask: (N_batch, 1, N_res, N_res) boolean tensor
                  True where valid (min_dist < d < max_dist) or i==j
        """
    ...
    def interleave_attention_mask(self,mask, repeat=4, causal=True):
        """
        Expand residue-level mask to token-level mask where each residue has `repeat` tokens.
        Apply fully causal restriction (no attending to any later tokens).
        """
        B, _, N, _ = mask.shape
        eye = torch.eye(N, dtype=torch.bool, device=mask.device).unsqueeze(0).unsqueeze(1)
        mask_full = mask | eye
        token_mask = (
            mask_full
            .repeat_interleave(repeat, dim=2)
            .repeat_interleave(repeat, dim=3)
        )
        if causal:
            total_tokens = N * repeat
            causal_mask = torch.tril(torch.ones((total_tokens, total_tokens), dtype=torch.bool, device=mask.device))
            token_mask = token_mask & causal_mask.unsqueeze(0).unsqueeze(1)
        return token_mask
    ...
    def predict_step(self, batch, batch_idx):
        ...
        for block in self.model.transformer.blocks:
            block.attn.k_cache = torch.zeros(N_batch,N_seq * 6,1536).to(self.device)
            block.attn.v_cache = torch.zeros(N_batch,N_seq * 6,1536).to(self.device)
        sequence_tokens = sequence_tokens[:,1:-1]
        structure_tokens = structure_tokens[:,1:-1]
        threeD = threeD[:,:,1]
        torsion_tokens = torch.ones(sequence_tokens.shape[0],sequence_tokens.shape[1] * 5).to(sequence_tokens.device) * 64
        torsion_tokens[:, ::5] = 65
        torsion_tokens = torsion_tokens.to(dtype=sequence_tokens.dtype)
        ...
        struc_attn_mask[:,:,1,0] = 1
        struc_attn_mask[:,:,0] = struc_attn_mask[:,:,1]
        struc_attn_mask = struc_attn_mask * seq_attn_mask
        mask_expanded = self.interleave_attention_mask(struc_attn_mask, repeat=5, causal=True)
        final_mask = torch.zeros(sequence_tokens.shape[0],1,sequence_tokens.shape[1] * 6,sequence_tokens.shape[1] * 6).to(self.device)
        final_mask[:,:,:sequence_tokens.shape[1],:sequence_tokens.shape[1]] = struc_attn_mask
        final_mask[:,:,sequence_tokens.shape[1]:,:sequence_tokens.shape[1]] = struc_attn_mask.repeat_interleave(5, dim=2)
        final_mask[:,:,sequence_tokens.shape[1]:,sequence_tokens.shape[1]:] = mask_expanded
        for j in range(N_seq):
            for i in range(5):
                logits = self.forward(seq_tokens=sequence_tokens, 
                                      struc_tokens=structure_tokens,
                                      torsion_tokens = torsion_tokens[:,j * 5 + i:j * 5 + i + 1],
                                      mask = final_mask,
                                      index = j * 5 + i)
                if sequence[j] in AA_TO_N and 0 <= i < AA_TO_N[sequence[j]]:
                    sample_token,prob_token,logprob_token = self.sample_i_with_temperature(logits)
                    #prob *= prob_token
                    #logprob += logprob_token
                    torsion_tokens[:,j * 5 + i + 1] = sample_token
        torsion_tokens = torsion_tokens.view(torsion_tokens.shape[0], -1, 5)[:, :, 1:].reshape(torsion_tokens.shape[0], -1)
        torsion = tokens_to_torsion_deg(torsion_tokens,n_bins = 64).detach().cpu().numpy()
...
def sample(lightning_model,trainer,protein_name=None,device="cuda"):
    ...
    merge_folder = f"all_atom_ARM/{protein_name}"
...
def run_experiment(protein_name = None):
    ...
    ckpt_path = hf_hub_download(
        ...
        filename="LangBoltz_sidechain_ARM.ckpt",
    )
